...
function initialCountdownSeconds(req: { remainingMs?: number; timeoutMs?: number; expiresAt?: string }): { countdown: number; total: number } {
    const total = Math.max(1, Math.ceil((req.timeoutMs ?? 60_000) / 1000));
    if (typeof req.remainingMs === 'number') {
        return { countdown: Math.max(0, Math.ceil(req.remainingMs / 1000)), total };
    }
    if (req.expiresAt) {
        const expiresAt = Date.parse(req.expiresAt);
        if (!Number.isNaN(expiresAt)) {
            return { countdown: Math.max(0, Math.ceil((expiresAt - Date.now()) / 1000)), total };
        }
    }
    return { countdown: total, total };
}
    ...
    useEffect(() => {
        ...
        for (const req of pendingPermissions) {
            if (prevStates[req.requestId]) {
                ...
                const initial = initialCountdownSeconds(req);
                newStates[req.requestId] = { ...initial, intervalId };
                changed = true;
            }
        }
            ...
            <div className="w-full max-w-md mx-4 max-h-[70vh] flex flex-col bg-white dark:bg-zinc-900 rounded-xl shadow-2xl border border-zinc-200 dark:border-zinc-700 overflow-hidden">
                ...
                <div className="overflow-y-auto flex-1 divide-y divide-zinc-100 dark:divide-zinc-800">
                    {pendingPermissions.map((req) => {
                        ...
                        const fallback = initialCountdownSeconds(req);
                        const countdown = state?.countdown ?? fallback.countdown;
                        const total = state?.total ?? fallback.total;
                                    ...
                                    <div className="flex-1 flex items-center gap-2">
                                        <div className="h-1 flex-1 bg-zinc-200 dark:bg-zinc-700 rounded-full overflow-hidden">
                                            <div
                                                ...
                                                style={{ width: `${Math.max(0, Math.min(100, (countdown / total) * 100))}%` }}
                                            />
                                        </div>
