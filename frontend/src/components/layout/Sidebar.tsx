...
import { FileChangesDashboard } from '@/components/dashboard/FileChangesDashboard';
import { sendToServer } from '@/api/stompClient';
import type { TaskState } from '@/types';

type TabType = 'sessions' | 'tasks' | 'files' | 'history' | 'sequence' | 'dag' | 'git' | 'complexity' | 'impact' | 'api-docs' | 'diagram' | 'code-path' | 'apos';
...
export function Sidebar({ className = '', isDrawerMode = false, defaultTab }: SidebarProps) {
    const [activeTab, setActiveTab] = useState<TabType>(() => {
        if (defaultTab && ['sessions','tasks','files','history','sequence','dag','git','complexity','impact','api-docs','diagram','code-path','apos'].includes(defaultTab)) {
    ...
    useEffect(() => {
        ...
        const valid = ['sessions','tasks','files','history','sequence','dag','git','complexity','impact','api-docs','diagram','code-path','apos'] as const;
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                {activeTab === 'history' && <FileChangesDashboard sessionId={activeSessionId ?? ''} />}
